	...
	<body>
		<header>
			<?php if (dobby_option('global_nav_layout') != 'alpha') { ?>
				<nav class="navbar navbar-expand-md fixed-top navbar-dark site-header" id="header-nav">
					<div class="container d-flex justify-content-between">
					  <?php if ( has_nav_menu('header_menu') ) {?>
					  <button class="navbar-toggler p-0 border-0 nav-list" id="navbutton" type="button" data-toggle="menu" data-target="#navbars" aria-controls="navbars" aria-expanded="false" aria-label="Toggle navigation">
			    ...
			    <nav class="navbar navbar-expand-md fixed-top navbar-dark site-header" id="header-nav">
	        		...
	        		<?php wp_nav_menu( array('theme_location' => 'header_menu', 'depth' => 2, 'container' => null, 'menu_class' => 'navbar-nav mr-auto', 'walker' => new WP_Bootstrap_Navwalker())); ?>
			            <form class="form-inline d-none d-lg-block" role="search" method="get" id="searchform" action="<?php echo home_url( '/' ); ?>">
			              <input class="form-control mr-sm-2" type="search" id="search" name="s" placeholder="请输入你要搜索的关键词" aria-label="Search">
