...
class dobby_widget_posts extends WP_Widget{

    function __construct(){
        $widget_ops = array(
        	
        	'classname' => 'dobby_widget_posts',
            'name'        => 'Dobby - 文章聚合',
            'description'=>'Dobby主题特色组件 - 文章聚合'
        );
        parent::__construct( false, false, $widget_ops );
    }
    ...
    function widget($args, $instance){
        ...
        $number = (!empty($instance['number'])) ? intval($instance['number']) : 5;
        ?>
        <section id="dobby_posts-2" class="widget_dobby_posts mb-3 clearfix">
            <h3><span>文章推荐</span></h3>
            <ul class="item-box">
            <?php $myposts = get_posts('numberposts='.$number.' & offset=0'); foreach($myposts as $post) : ?>
            <li class="item clearfix">
            <a href="<?php echo get_permalink($post->ID); ?>" class="text-dark d-block" title="<?php echo $post->post_title;?>">
			<div class="item-img">
			<img width="480" height="300" src="https://image.jecho.cn/w.jpg" class="thumbnail" alt="<?php echo $post->post_title;?>" style="display: inline;">
			</div>
            <div class="item-content">
            <p class="item-title">"<?php echo $post->post_title;?>"</p>
            <p class="item-excerpt"><?php echo wp_trim_words(get_the_excerpt(),30); ?></p>
            </div>
            </a>
            </li>
            <?php endforeach; ?>
...
function dobby_register_widgets(){
	...
	register_widget('dobby_widget_posts');
}
?>